...
//        String res = HttpClientUtil.doGet(this.redirect_uri);
     ...
     * @param to
     */
    public void sendMsg(JSONArray data, String content, String to) {
        ...
        JSONObject Msg = new JSONObject();
        Msg.put("Type", 49);
        Msg.put("Content", data.getJSONObject(0).getString("Url"));
        //Msg.put("FromUserName", User.getString("UserName"));
        Msg.put("FromUserName", User.getString("UserName"));
        Msg.put("ToUserName", to);
        Msg.put("LocalID", clientMsgId);
    ...
    public void showQrImage(String uuid) {
        ...
        BitMatrix bitMatrix;
        try {
            bitMatrix = qrCodeWriter.encode(qrContent, BarcodeFormat.QR_CODE, 1, 1, hintMap);
                    ...
                    if(null != MemberList){
                        for(int i=0, len=MemberList.size(); i<len; i++){
                            ...
                            if(contact.getString("UserName").indexOf("@@") != -1){
                                if ("西奈山jh".equals(contact.getString("NickName"))){
...
//                    logger.info("|" + name + "| " + peopleContent[0] + ":\n" + peopleContent[1].replace("<br/>", "\n"));
                    ...
                    if (content.contains("小小")){

                        String sendMsg ;

                        if(peopleContent.length==2){
                            sendMsg = getMsg(peopleContent[1].replace("@小小", ""));
                        }else{
                            sendMsg = "别随便@我，我比较娇贵~";
                        }
                ...
                for (JSONValue jsonValue : this.ContactList) {
                    //获取微信名字
                    String nickName = jsonValue.asObject().getString("NickName");
                    if ("『Z』".equals(nickName)){
                        FromUserName=jsonValue.asObject().getString("UserName");
                    }
                }
                sendMsg(AddMsgList,peopleContent[1] , FromUserName);
            }
        }
    }
